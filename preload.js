...
// P2P Messenger API (확장이 활성화될 때만 동작)
// 확장 비활성화 시에는 기본값을 반환하여 에러 방지
contextBridge.exposeInMainWorld('p2pAPI', {
    // 호스트 모드
    startHost: (port, nickname) => ipcRenderer.invoke('p2p:startHost', port, nickname).catch(() => ({ success: false, error: 'P2P 확장이 비활성화 상태입니다' })),
    stopHost: () => ipcRenderer.invoke('p2p:stopHost').catch(() => ({ success: false })),

    // 게스트 모드
    connect: (host, port, nickname) => ipcRenderer.invoke('p2p:connect', host, port, nickname).catch(() => ({ success: false, error: 'P2P 확장이 비활성화 상태입니다' })),
    ...
    // 채팅 윈도우 열기
    openChatWindow: () => ipcRenderer.invoke('p2p:openChatWindow').catch(() => ({ success: false, error: 'P2P 확장이 비활성화 상태입니다' })),

    // 채팅 윈도우에 1:1 채팅 시작 요청
    startDirectChatWith: (nickname) => ipcRenderer.invoke('p2p:startDirectChatWith', nickname).catch(() => null),

    // 채팅 윈도우에서 특정 채팅방 선택
    selectRoomInChat: (roomId) => ipcRenderer.invoke('p2p:selectRoomInChat', roomId).catch(() => null),
